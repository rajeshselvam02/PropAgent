...
fastify.get('/api/analytics/time-to-conversion', async (req, reply) => {
  const { from, to, groupBy = 'source', tenantId } = req.query as any;
  ...
  if (precomputed.rows.length > 0) {
    reply.send({
      ...
      data: {
        period: { from: startDate, to: endDate },
        source: 'precomputed',
        leaderboard: precomputed.rows.map((row: any) => ({
...
fastify.get('/api/analytics/source-roi', async (req, reply) => {
  ...
  if (precomputed.rows.length > 0) {
    const data = precomputed.rows.map((row: any) => ({
      source: row.source,
...
fastify.get('/api/analytics/summary', async (req, reply) => {
  ...
  const totalLeads = leadsByStatus.rows.reduce((sum: number, r: any) => sum + parseInt(r.count), 0);
  const convertedLeads = parseInt(leadsByStatus.rows.find((r: any) => r.status === 'converted')?.count || '0');
  ...
  reply.send({
    ...
    data: {
      ...
      leads: {
        total: totalLeads,
        byStatus: Object.fromEntries(leadsByStatus.rows.map((r: any) => [r.status, parseInt(r.count)])),
        bySource: Object.fromEntries(leadsBySource.rows.map((r: any) => [r.source, parseInt(r.count)]))
      },
      conversions: {
...
fastify.get('/api/analytics/team-metrics', async (req, reply) => {
  const { teamId, from, to, tenantId } = req.query as any;
  ...
  const individualMetrics = await db.query(`
    SELECT 
      a.id,
      a.name,
      COUNT(DISTINCT l.id) as leads_assigned,
  ...
  reply.send({
    ...
    data: {
      ...
      individuals: individualMetrics.rows.map((r: any) => ({
        agentId: r.id,
        name: r.name,
