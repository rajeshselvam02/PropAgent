...
async function handleSubscriptionUpdated(subscription: any) {
  ...
  const subResult = await db.query(
    'SELECT * FROM tenant_subscriptions WHERE stripe_subscription_id = $1',
    [subscriptionId]
  );
  ...
  const planResult = await db.query('SELECT name FROM plans WHERE id = $1', [planId]);
  const planName = planResult.rows[0]?.name?.toLowerCase() || 'unknown';

  // Update tenant plan
  await db.query(
    'UPDATE tenants SET plan = $1, updated_at = NOW() WHERE id = $2',
    [planName, existing.tenant_id]
  );
...
async function handleSubscriptionDeleted(subscription: any) {
  ...
  await db.query(
    "UPDATE tenants SET plan = 'trial', updated_at = NOW() WHERE id = $1",
    [existing.tenant_id]
  );
...
async function handlePaymentFailed(invoice: any) {
  ...
  const subResult = await db.query(
    'SELECT * FROM tenant_subscriptions WHERE stripe_customer_id = $1',
    [customerId]
  );
