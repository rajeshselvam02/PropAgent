 ...
 */
export async function processWhatsAppSend(job: Job<WhatsAppSendData>) {
 ...
 */
export async function processQualificationStep(job: Job<QualificationStepData>) {
 ...
 */
export async function calculateIntentScore(leadId: string, responses: Record<string, string>) {
  let score = 0;
  
  // Budget (30 points)
  const budget = responses.budget;
  if (budget?.includes('1Cr')) score += 30;
  else if (budget?.includes('60L')) score += 20;
  else if (budget) score += 10;
  ...
  // Location (20 points)
  if (responses.location) score += 20;
  
  // Purpose (15 points)
  const purpose = responses.purpose;
  if (purpose === 'Self-use') score += 15;
  else if (purpose === 'Investment') score += 12;
  else if (purpose) score += 8;
