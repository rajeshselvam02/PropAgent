 ...
 */
export async function processAnalyticsRollup(job: Job<AnalyticsRollupData>) {
  ...
  const allTenants = new Set([
    ...newLeadsResult.rows.map((r: any) => r.tenant_id),
    ...intentResult.rows.map((r: any) => r.tenant_id),
  ]);
  
  for (const tid of allTenants) {
    const newLeads = newLeadsResult.rows.find((r: any) => r.tenant_id === tid)?.count || 0;
    const intent = intentResult.rows.find((r: any) => r.tenant_id === tid) || { hot: 0, warm: 0, cold: 0 };
    const status = statusResult.rows.find((r: any) => r.tenant_id === tid) || { qualified: 0, visits: 0, converted: 0, lost: 0 };
    const interactions = interactionsResult.rows.find((r: any) => r.tenant_id === tid) || { calls: 0, whatsapp: 0, emails: 0 };
 ...
 */
export async function processAgentPerformance(job: Job<AgentPerformanceData>) {
  ...
  if (avgResponseTimeSeconds < 300) responseTimeScore = 100;
  else if (avgResponseTimeSeconds < 1800) responseTimeScore = 80;
  else if (avgResponseTimeSeconds < 7200) responseTimeScore = 60;
  else if (avgResponseTimeSeconds < 86400) responseTimeScore = 40;
 ...
 */
export async function computeFunnelMetrics(tenantId: string, startDate: string, endDate: string) {
