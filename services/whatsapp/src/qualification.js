...
const qualificationSessions = new Map();
...
async function processMessage(phone, text) {
    ...
    if (!session) {
        ...
        const leadResult = await db_1.default.query('SELECT * FROM leads WHERE phone = $1', [phone]);
      ...
      `, [phone]);
        ...
        const leadId = leadResult.rows[0]?.id || (await db_1.default.query('SELECT id FROM leads WHERE phone = $1', [phone])).rows[0].id;
        session = {
            leadId,
            phone,
    ...
    switch (session.currentState) {
        ...
        case 'timeline':
            ...
            if (text.includes('asap') || text.includes('1-3'))
                session.intentScore += 30;
...
async function completeQualification(session) {
