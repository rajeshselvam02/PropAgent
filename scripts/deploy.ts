...
async function runStep(name: string, cmd: string): Promise<{ success: boolean; output: string }> {
  console.log(`\n[STEP] ${name}`);
  console.log(`[CMD] ${cmd}`);
  try {
...
function generateReport(results: { step: string; success: boolean; output: string }[], successful: number, failed: number): string {
