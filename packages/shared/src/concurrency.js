...
exports.CONFLICT_CODES = void 0;
exports.parseIfMatch = parseIfMatch;
exports.formatETag = formatETag;
exports.concurrencyMiddleware = concurrencyMiddleware;
exports.buildVersionedUpdate = buildVersionedUpdate;
exports.executeVersionedUpdate = executeVersionedUpdate;
exports.addETagHeader = addETagHeader;
exports.concurrencyAwareGet = concurrencyAwareGet;
exports.batchVersionCheck = batchVersionCheck;
/**
 * Concurrency error codes
 */
exports.CONFLICT_CODES = {
 ...
 */
function concurrencyMiddleware(entityType, getVersion) {
        ...
        try {
            ...
            if (currentVersion !== expectedVersion) {
                reply.code(409).send({
                    ...
                    code: exports.CONFLICT_CODES.VERSION_MISMATCH,
                    currentVersion,
                    expectedVersion,
            ...
            req._expectedVersion = expectedVersion;
            return true;
        }
        catch (err) {
 ...
 */
function buildVersionedUpdate(table, id, tenantId, expectedVersion, updates) {
 ...
 */
async function executeVersionedUpdate(db, table, id, tenantId, expectedVersion, updates) {
    ...
    if (result.rows.length === 0) {
        ...
        const check = await db.query(`SELECT version FROM ${table} WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
 ...
 */
function addETagHeader(reply, version) {
 ...
 */
async function concurrencyAwareGet(db, table, id, tenantId, reply) {
    const result = await db.query(`SELECT * FROM ${table} WHERE id = $1 AND tenant_id = $2`, [id, tenantId]);
    if (result.rows.length === 0)
        return null;
 ...
 */
async function batchVersionCheck(db, table, updates, tenantId) {
    const mismatches = new Map();
    if (updates.length === 0)
        return mismatches;
    const ids = updates.map(u => u.id);
    const result = await db.query(`SELECT id, version FROM ${table} WHERE id = ANY($1) AND tenant_id = $2`, [ids, tenantId]);
